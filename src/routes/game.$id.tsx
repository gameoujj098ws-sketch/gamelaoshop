...
interface Pkg {
  ...
  original_price: number | null; is_best_seller: boolean;
}

export const Route = createFileRoute("/game/$id")({
  head: () => ({
    meta: [
      { title: "ເຕີມເກມ — Gamelao" },
      { name: "description", content: "ເລືອກແພັກເກັດ ແລະ ເຕີມເກມທັນທີ ລາຄາຖືກ ໄວທັນໃຈ" },
      { property: "og:title", content: "ເຕີມເກມ — Gamelao" },
      { property: "og:description", content: "ເລືອກແພັກເກັດ ແລະ ເຕີມເກມທັນທີ ລາຄາຖືກ ໄວທັນໃຈ" },
...
function GamePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: c }, { data: f }, { data: p }] = await Promise.all([
        supabase.from("categories").select("id, name, image_url").eq("id", id).maybeSingle(),
  ...
  if (!cat) {
    return (
      <AppShell>
        <div className="px-4 py-10 text-center text-sm text-muted-foreground">ບໍ່ພົບເກມນີ້</div>
      </AppShell>
    );
  }
    ...
    <AppShell>
      <div className="px-4 space-y-4 pb-6">
        ...
        <div className="card-tile p-5 flex flex-col items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-primary underline underline-offset-4">{cat.name}</h1>
        </div>
        ...
        <div className="card-tile p-4 space-y-3">
          <div className="flex items-center gap-2">
            <StepBadge n={1} />
            <h2 className="font-bold">ຂໍ້ມູນຜູ້ຫຼິ້ນ</h2>
          </div>
          {fields.length === 0 ? (
        ...
        <div className="card-tile p-4 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">ກະລຸນາເລືອກແພັກເກັດທີ່ຕ້ອງການ</p>
          {pkgs.length === 0 ? (
              ...
              {pkgs.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    {p.image_url && (
                      <img src={p.image_url} alt={p.name} className="size-12 object-contain" />
                    )}
                    ...
                    {off > 0 && (
                      <div className="text-[10px] text-muted-foreground line-through">{formatKip(p.original_price!)} ກີບ</div>
                    )}
        ...
        <div className="card-tile p-4 space-y-3">
          ...
          <p className="text-[11px] leading-relaxed text-muted-foreground flex gap-2">
            ...
            ກະລຸນາກວດສອບຂໍ້ມູນຜູ້ຫຼິ້ນ ແລະ ແພັກເກັດໃຫ້ຖືກຕ້ອງກ່ອນຢືນຢັນ. ຫຼັງຈາກຢືນຢັນແລ້ວຈະບໍ່ສາມາດຍົກເລີກໄດ້.
          </p>
