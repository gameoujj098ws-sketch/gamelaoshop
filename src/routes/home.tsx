    ...
    <AppShell>
      <div className="space-y-4">
        ...
        <div className="px-4 space-y-2">
          <h2 className="text-sm font-bold flex items-center gap-2"><Layers className="size-4 text-primary" /> ໝວດໝູ່ສິນຄ້າທົ່ວໄປ</h2>
          {loading ? (
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              <button onClick={() => setCat(null)}
                className={`rounded-full px-4 py-1.5 text-xs whitespace-nowrap border ${cat === null ? "border-primary bg-primary/10 text-primary" : "border-border/60 bg-surface text-muted-foreground"}`}>
                ທັງໝົດ
              </button>
              {cats.map((c) => (
                <button key={c.id} onClick={() => setCat(c.id)}
                  className={`rounded-full px-4 py-1.5 text-xs whitespace-nowrap border inline-flex items-center gap-1.5 ${cat === c.id ? "border-primary bg-primary/10 text-primary" : "border-border/60 bg-surface text-muted-foreground"}`}>
                  {c.image_url && <img src={c.image_url} alt={c.name} className="size-5 rounded-full object-cover" />}
                  {c.name}
                </button>
              ))}
            </div>
